...
module.exports = {
  name: "slowmode",
  aliases: ["sm", "modolento", "ml"],
  category: "Moderation",
  description: "Slow mode on a text channel",
  run: async (client, message, args) => {

        if (!message.member.hasPermission('MANAGE_CHANNELS')) return message.channel.send('You do not have **MANAGE_CHANNELS** permission!').then(m => m.delete({ timeout: 6000 }));
        ...
        if (time >= 21601) return message.channel.send('That slowmode limit is too high, please enter anything lower than 6 hours.').then(m => m.delete({ timeout: 5000 }));
